...
static SFU_ErrorStatus SFU_BOOT_Init(void)
{
  ...
  /* The COM module initialization is required here only if the trace is enabled.
     The COM and Loader modules will be initialized on new FW download. */
#if defined(SFU_DEBUG_MODE) || defined(SFU_TEST_PROTECTION)
...
static SFU_ErrorStatus SFU_BOOT_DeInit(void)
{
  ...
  if (SFU_LEDs_DeInit() != SFU_SUCCESS)
  {
    return SFU_ERROR;
  }
...
static void SFU_BOOT_SM_DownloadNewUserFw(void)
{
  ...
  SFU_LEDs_Init(BLINKING_ALTERNATE_LED);
...
static void SFU_BOOT_SM_InstallNewUserFw(void)
{
  ...
  SFU_LEDs_Init(BLINKING_SYNCHRONIZED_LED);
