...
#else
#if defined(SFU_TEST_PROTECTION) || defined(SFU_BACKUP_REG_ENABLE)
  if (SFU_LL_RTC_Init() != SFU_SUCCESS)
  {
    e_ret_status = SFU_ERROR;
  }

#endif /* SFU_TEST_PROTECTION || SFU_BACKUP_REG_ENABLE */
#endif /* SFU_TAMPER_PROTECT_ENABLE */
                                                  ...
                                                  SFU_BoolTypeDef *pbIsProtectionToBeApplied)
{
  ...
  else
  {
    ...
    psFlashOptionBytes->USERType        = OB_USER_BFB2;
...
SFU_ErrorStatus SFU_LL_SECU_SetProtectionMPU_UserApp(void)
{
  ...
  MPU_InitStruct.BaseAddress          = APP_PROTECT_MPU_AREA_HIDE_START;
  MPU_InitStruct.Size                 = APP_PROTECT_MPU_AREA_HIDE_SIZE;
  MPU_InitStruct.SubRegionDisable     = APP_PROTECT_MPU_AREA_HIDE_SREG;
  MPU_InitStruct.AccessPermission     = APP_PROTECT_MPU_AREA_HIDE_PERM;
  MPU_InitStruct.DisableExec          = APP_PROTECT_MPU_AREA_HIDE_EXEC;
  MPU_InitStruct.IsShareable          = MPU_ACCESS_NOT_SHAREABLE;
  MPU_InitStruct.IsBufferable         = MPU_ACCESS_NOT_BUFFERABLE;
  MPU_InitStruct.IsCacheable          = MPU_ACCESS_CACHEABLE;
  MPU_InitStruct.TypeExtField         = MPU_TEX_LEVEL0;
