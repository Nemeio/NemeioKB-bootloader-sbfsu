...
# arg5 is the version
# arg6 is the elf file path+name [output]
# arg7 when present forces "bigelf" generation
projectdir=$1
FileName=${3##*/}
execname=${FileName%.*}
elf=$2
...
current_directory=`pwd`
cd $SecureEngine/../../../../../../Middlewares/ST/STM32_Secure_Engine/Utilities/KeysAndImages
basedir=`pwd`
cd $current_directory
...
if [ $ret -eq 0 ]; then
  ...
  ret=$?
  if [ $ret -eq 0 ]; then 
    module="[SECBOOT_ECCDSA_WITH_AES128_CBC_SHA256]"
    
    if [ "$USING_AZURE_KEYVAULT" = true ] ; then
      echo "$module Using Azure keyvault to perform the signature"
    else
      echo "$module Using local private key to perform the signature"
    fi
    
    if [ "$USING_AZURE_KEYVAULT" = true ] ; then
      command=$cmd" "$prepareimage" pack -m "$magic" --azure_keyvault_url "$NEMEIO_AZURE_KEYVAULT_URL" --azure_keyvault_key_name "$NEMEIO_AZURE_KEYVAULT_STM32_KEYNAME" --azure_keyvault_public_key "$NEMEIO_AZURE_KEYVAULT_STM32_PUBKEY_PEM_PATH" -r 28 -v "$version" -i "$iv" -f "$sfu" -t "$sign" "$sfb" -o "$offset
    else
    ...
    if [ $ret -eq 0 ]; then
      if [ "$USING_AZURE_KEYVAULT" = true ] ; then
        command=$cmd" "$prepareimage" header -m "$magic"  --azure_keyvault_url "$NEMEIO_AZURE_KEYVAULT_URL" --azure_keyvault_key_name "$NEMEIO_AZURE_KEYVAULT_STM32_KEYNAME" --azure_keyvault_public_key "$NEMEIO_AZURE_KEYVAULT_STM32_PUBKEY_PEM_PATH" -r 28 -v "$version" -i "$iv" -f "$sfu" -t "$sign" -o "$offset" "$headerbin
      else
...
if [ $ret -eq 0 ]; then
  ...
  rm $sfu
  rm $headerbin
  echo "displaying output.txt for debugging purpose"
  cat $projectdir"/output.txt"
