...
if [ "$USING_AZURE_KEYVAULT" = true ] ; then
  echo "$module Using Azure keyvault to extract public key in bootloader"
else
  echo "$module Using local private key in bootloader"
fi
...
if [ $ret -eq 0 ]; then
  ...
  if [ $type != "vide" ]; then
    if [ "$USING_AZURE_KEYVAULT" = true ] ; then
      ecckey=${NEMEIO_AZURE_KEYVAULT_STM32_PUBKEY_PEM_PATH}
    else
    ...
    # Azure is not supported for key 2 and 3
