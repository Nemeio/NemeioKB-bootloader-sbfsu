...
class ECDSA256P1Public():
    def __init__(self, key):
        """Construct an ECDSA P-256 public key"""
        self.key = key
...
class ECDSA256P1Private():
    ...
    def trans(self,section, name, end, assembly, version):
        ...
        pub = ECDSA256P1Public(vk)
        return pub.trans(section, name, end, assembly, version)
    ...
    if len(pem) == 16:
        ...
        if not ecdsa:
            try:
                ...
                ecdsa = ECDSA256P1Public(key)
            except:
                pass

        # validate type
        if ecdsa and ecdsa.get_curve() == 'NIST256p':
            return ecdsa
        else:
