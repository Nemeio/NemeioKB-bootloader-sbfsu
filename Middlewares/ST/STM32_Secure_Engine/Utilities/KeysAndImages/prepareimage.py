...
from azure_keyvault_signer.azure_keyvault_signer import AzureKeyvaultSigner
...
def do_header_lib(args):
    ...
    if (args.cert_fw_leaf or args.cert_fw_inter):
        ...
        header += b'\0'*padding
    if args.azure_keyvault_url and args.azure_keyvault_key_name and args.azure_keyvault_public_key:
        print("Signing with Azure Key Vault instead of local private key!")
        az_signer = AzureKeyvaultSigner(vault_uri=args.azure_keyvault_url, key_name=args.azure_keyvault_key_name)
        signature = az_signer.keyvault_sign(header, hashlib.sha256, signature_algorighm=SignatureAlgorithm.es256)
        
        with open(args.azure_keyvault_public_key, 'r') as f:
            az_pubkey = f.read()
            vk_test = VerifyingKey.from_pem(az_pubkey)
        
        try:
            if vk_test.verify(signature, header, hashfunc=hashlib.sha256) : 
                print(f"Verification OK")
                header +=pack(str(len(signature))+'s',signature)
        except BadSignatureError as err:
            print(f"Signature Verification failed : {err}")
            exit(1)
...
def args():
    ...
    head.add_argument('-k', '--key', metavar='filename', required = False)
    head.add_argument('--azure_keyvault_url', type=str, required=False)
    head.add_argument('--azure_keyvault_key_name', type=str, metavar='filename', required=False)
    head.add_argument('--azure_keyvault_public_key', metavar='filename',required=False)
    ...
    pack.add_argument('-k', '--key', metavar='filename', required = False)
    pack.add_argument('--azure_keyvault_url', type=str, required=False)
    pack.add_argument('--azure_keyvault_key_name', metavar=str, required=False)
    pack.add_argument('--azure_keyvault_public_key', metavar='filename',required=False)
